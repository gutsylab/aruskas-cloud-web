        ...
        <div class="row">
            <div class="col-md-6">
                <form method="POST" id="form-edit" class="form-horizontal"
                    action="{{ route('cash-accounts.update', [
                    ...
                    @if ($id > 0)
                        @if ($account->journalLines->count() > 0)
                            <div class="alert alert-warning px-4 pb-0 pt-4 mb-4">
                                ...
                                <p>Akun kas ini sudah memiliki transaksi, proses hapus akan dibatalkan.</p>
                            </div>
                        @endif
                    @endif
                    ...
                    <div class="d-flex justify-content-end gap-3 my-5">
                        ...
                        <button type="button" id="btn-submit" onclick="doSubmit('form-edit')" class="btn btn-primary">
                            ...
                            <span id="btn-text">Simpan Akun Kas</span>
                        </button>
                    </div>
                </form>
            </div>
...
@section('js')
    ...
    <script>
        ...
        function confirmArchive(id) {
            confirmAlert(
                'Arsip Akun Kas',
                'Apakah anda yakin ingin mengarsipkan akun kas ini?',
                'warning',
                function() {
                    window.location.href =
                        '{{ route('cash-accounts.archive', ['tenant_id' => $tenant->tenant_id, 'cash_account' => ':id']) }}'
                        .replace(':id', id);
                }
            );
        }
        ...
        function confirmActivate(id) {
            confirmAlert(
                'Aktifkan Akun Kas',
                'Apakah anda yakin ingin mengaktifkan akun kas ini?',
                'question',
                function() {
                    window.location.href =
                        '{{ route('cash-accounts.active', ['tenant_id' => $tenant->tenant_id, 'cash_account' => ':id']) }}'
                        .replace(':id', id);
                }
            );
        }
        ...
        function confirmDelete(id) {
            confirmAlert(
                'Hapus Akun Kas',
                'Apakah anda yakin ingin menghapus akun kas ini? Data yang sudah dihapus tidak dapat dikembalikan lagi.',
                'warning',
                function() {
                    $.ajax({
                        url: '{{ route('cash-accounts.destroy', ['tenant_id' => $tenant->tenant_id, 'cash_account' => ':id']) }}'
                            .replace(':id', id),
                        type: 'POST',
                        data: {
                            _method: 'DELETE',
                        ...
                        success: function(response) {
                            ...
                            successAlert(response.message, function() {
                                window.location.href =
                                    '{{ route('cash-accounts.index', ['tenant_id' => $tenant->tenant_id]) }}';
                            });
                        },
                        error: function(xhr) {
                            errorAlert(`Gagal menghapus akun. Error : ${xhr.status} ${xhr.statusText}`);
                        }
                    });
                });
        }
